...
//' rec.ccdata.chisq = esm_chisq( rec.ccdata[,which(MAF <= 0.05)], status, 50 )
// [[Rcpp::export]]
NumericVector esm_chisq( const IntegerMatrix & ccdata,
			 const IntegerVector & ccstatus,
			 const unsigned & k)
{
  return( esm( chisq_per_marker(ccdata,ccstatus), k ) );
}
